  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto custom-scrollbar pr-1">
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl border text-left flex items-center justify-between transition-colors cursor-pointer ${
                selectedContestId === contest.contestId
                  ...
                  : "bg-gray-900/40 border-gray-800 text-gray-400 hover:bg-gray-900/60 hover:text-gray-200"
              }`}
            >
              <div className="space-y-1">
                <div className="font-mono text-sm font-semibold flex items-center gap-1.5 text-white">
                  {contest.contestId}
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-gray-800 text-gray-400 font-normal">
        ...
        {/* Details Card */}
        <div className="bg-gray-900/20 border border-gray-800 rounded-xl p-3 flex flex-col h-[300px]">
          {selectedContest ? (
            <div className="flex flex-col h-full">
              <div className="border-b border-gray-800 pb-2 mb-2">
                <h3 className="font-mono font-bold text-sm text-purple-400">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1.5">
                {selectedContest.rankings.map((user, idx) => (
                  <div
                    key={user.username}
                    className="flex items-center justify-between p-2 rounded-lg bg-gray-900/40 border border-gray-800/40 text-xs"
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded bg-gray-800 text-gray-400 flex items-center justify-center font-mono font-bold text-[10px]">
                        {idx + 1}
                      </span>
                      <img
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 text-sm text-center">
              ...
              <span>Select a contest from the list to view its final participant standings.</span>
            </div>
          )}
        </div>
      </div>
